...
In the previous practice, we show how to fit spatial Poisson mixed models for high-dimensional areal data using the R package `bigDM`. Here, we describe how to use this package to fit order-free multivariate scalable Bayesian models to smooth mortality (or incidence) risks of several diseases simultaneously [Vicente et al, (2023)](https://doi.org/10.1007/s11222-023-10263-x).
...
Again, we propose to divide the data into the $D=15$ Autonomous Regions of Spain (`region` variable of the `Carto_SpainMUN` object). The *disjoint* and *k-order neighbourhood models* with an iCAR prior distribution are fitted using the `MCAR_INLA()` function as:
...
Maps of posterior medians of between-disease correlations and standard deviation (in brackets) for the different subdivisions obtained with the 1st-order neighbourhood partition model. Correlations between lung and colorectal cancer are displayed on the left ($\rho_{1,2}$), the central map displays the correlations between lung and stomach cancer ($\rho_{1,3}$) and the map on the right displays the correlation between colorectal and stomach cancer ($\rho_{2,3}$).
...
cor.values <- data.frame(rho12.median=model$summary.cor.partition[rho12,"0.5quant"],
                         rho13.median=model$summary.cor.partition[rho13,"0.5quant"],
                         rho23.median=model$summary.cor.partition[rho23,"0.5quant"],
                         rho12.sd=model$summary.cor.partition[rho12,"sd"],
                         rho13.sd=model$summary.cor.partition[rho13,"sd"],
                         rho23.sd=model$summary.cor.partition[rho23,"sd"])
...
## Text ##
carto$rho.12 <- round(carto$rho12.median,2)
carto$rho.13 <- round(carto$rho13.median,2)
carto$rho.23 <- round(carto$rho23.median,2)
carto$SD.rho12 <- paste0("(",round(carto$rho12.sd,2),")")
carto$SD.rho13 <- paste0("(",round(carto$rho13.sd,2),")")
carto$SD.rho23 <- paste0("(",round(carto$rho23.sd,2),")")

aux.name <- c(expression(rho[1.2]),expression(rho[1.3]),expression(rho[2.3]))
pos.x <- rep(0, 15)
pos.x[c(2,5)] <- c(-0.2,0.45)
pos.y <- rep(0.25, 15)
pos.y[c(2,5,7)] <- c(0,0.3,0.5)
pos.y.ic <- rep(-0.25, 15)
pos.y.ic[c(2,5,7)] <- c(-0.5,-0.2, 0)

Map <- vector("list", J*(J-1)/2)
names(Map) <-  paste("rho",apply(combn(J,2), 2, function(x) paste0(x, collapse="")),sep="")

for(j in seq(length(Map))){
  Map[[j]] <- 
    tm_shape(carto) +
    tm_polygons(col=paste(names(Map)[j],"median",sep="."),
                palette=paleta, border.alpha=0, title="", legend.show=T, legend.reverse=T,
                style="fixed", breaks=values, interval.closure="left") +
    tm_layout(main.title="", 
              main.title.size = 0.85,
              main.title.position="center",
              legend.outside=F, legend.outside.position="right", legend.frame=F, legend.outside.size=0.3,
              panel.labels=aux.name[j],
              panel.label.bg.color="lightskyblue3", bg.color="aliceblue",
              outer.margins=c(0.02,0.01,0.02,0.01)) + 
    tm_shape(carto) + tm_borders(col="gray48", lwd=1) + 
    tm_text(paste("rho",apply(combn(J,2), 2, function(x) paste0(x, collapse="")),sep=".")[j],
            size=0.7, fontface="bold", legend.size.show=FALSE, 
            legend.col.show=FALSE, xmod=pos.x, ymod=pos.y) + 
    tm_shape(carto) + 
    tm_text(paste("SD.rho",apply(combn(J,2), 2, function(x) paste0(x, collapse="")),sep="")[j],
            size=0.5, fontface="bold", legend.size.show=FALSE, 
            legend.col.show=FALSE, xmod=pos.x, ymod=pos.y.ic)
}
...
print(Map[[3]], vp=viewport(layout.pos.row = 1, layout.pos.col = 3))
```
